...
public enum IMaterial {
	...
	private static HashMap<String, IMaterial> cachedIDSearch = new HashMap<>();
	public static IMaterial requestIMaterial(String idkey){
		if(cachedIDSearch.containsKey(idkey.toUpperCase())){
			return cachedIDSearch.get(idkey.toUpperCase());
		}
		for(IMaterial mat:IMaterial.values()){
			if(idkey.toUpperCase().equals((mat.id+""))){
				cachedIDSearch.put(mat.id+"",mat);
				return mat;
			}
		}
		return null;
	 ...
	 * key 所需查找物品的命名key,格式xxx:xxx或xxx。
	 */
	public static Material fromID(String idkey){
		try {
